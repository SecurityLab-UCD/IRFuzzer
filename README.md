...
It's an known issue that `Target/AIE/MCTargetDesc/AIEMCFormats.cpp` will take a long time to compile. A function in it `__cxx_global_var_init()` will cause the optimizer to run for a really long time. It is an interesting bug, but we haven't had time to fix it.
